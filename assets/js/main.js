(function ($) {
  ...
  // Ajax Search Fetch
  $("input#keyword").bind("change paste keyup", function (e) {
    e.preventDefault();

    const $current = $(this),
      keyword = $current.val();

    if (keyword.length !== 0 && keyword.length < 3) return false;
  ...
  $(function () {
    ...
    $(window).load(function () {
      ...
      if (mediaQuery.matches) {
        $(".allaround-upsell-items ul.products").slick();
      }
    });
...
*/
    var childCount = $(".alarnd--select-opt-header").children().length;
    var parentWidth = childCount * 79.8;
    var minimumWidth = 600;
    var finalWidth = Math.max(parentWidth, minimumWidth);
    $("#alarnd__select_options_info").css("width", finalWidth + "px");
