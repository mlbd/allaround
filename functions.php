...
if ( ! function_exists( 'hello_elementor_scripts_styles' ) ) {
	...
	function hello_elementor_scripts_styles() {
        ...
        wp_enqueue_script( 'search-js', get_template_directory_uri() . '/assets/js/classie.js', array('jquery'), HELLO_ELEMENTOR_VERSION, true );
        wp_enqueue_script( 'search-int', get_template_directory_uri() . '/assets/js/search-int.js', array('jquery'), HELLO_ELEMENTOR_VERSION, true );
        ...
        wp_localize_script( 'allaround-main', 'allaround_vars',
            array( 
...
function ml_update_cart_after_removeitem( $cart_item_key, $cart ) {
    ...
    if( ! empty( $alarnd_group_id ) ) {
        ...
        foreach ( $cart->cart_contents as $i_cart_item_key => $cart_item ) {
            $item_group_id = $cart_item['alarnd_group_id'] ;
    ...
    <div class="alarnd--cart-inner">
            ...
            <div class="alarnd--single-cart-row">
                ...
                <div class="alarnd--sizes-wrapper alarnd--sizes-wrapper-b-15">
                    ...
                    <div class="alarnd--sizes-wrap">
                        ...
                        <label for="alarnd__size_<?php echo strtolower( $size ); ?>"><?php echo esc_html( $size ); ?></label>
                        </div>
                        <?php endif; ?>
                        <?php endforeach; ?>
                    </div>
        ...
        <?php if( ! empty( $group_enable ) && ! empty( $colors ) ) : ?>
                ...
                <div class="alarnd--colors-wrapper">
                    <div class="alarnd--colors-wrap">
                        <?php foreach( $colors as $key => $color ) : ?>
                            <input type="radio" name="alarnd__color" id="alarnd__color_<?php echo esc_html( $color['title'] ); ?>" value="<?php echo esc_html( $color['title'] ); ?>" <?php echo 0 == $key ? 'checked' : ''; ?>>
            ...
            <div class="alarnd--button-groups">
                <button type="button" class="alarnd_view_select button alt"><?php esc_html_e( 'בחר גדלים וכמות', 'hello-elementor' ); ?></button>
                <button type="button" class="alarnd_view_pricing button alt"><?php esc_html_e( 'התמחור שלנו', 'hello-elementor' ); ?></button>
            </div>
        <?php endif; ?>
        <div class="alarnd--single-cart-row alarnd--single-cart-price">
            <span class="alarnd--total-price"><?php echo wc_price($product->get_regular_price(), array('decimals' => 0)); ?></span>
...
function alarnd_form_after() {
    ...
    <div id="alarnd__select_options_info" class="mfp-hide white-popup-block alarnd--slect-opt-modal alarnd--info-modal">
        <div class="alarnd--modal-inner alarnd--modal-chart-info">
            ...
            <form id="alarnd__select_options_form" class="cart" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" data-settings='<?php echo wp_json_encode( $json_data ); ?>' enctype='multipart/form-data'>
            ...
            <div class="alarnd--price-show-wrap">
                <div class="alarnd--single-cart-row alarnd--single-cart-price">
                    ...
                    <button type="submit" name="add-to-cart" id="alarnd__group_submit" value="<?php echo esc_attr( $product->get_id() ); ?>" class="single_add_to_cart_button button alt ml_add_loading ml_add_to_cart_trigger"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
                </div>
...
function alarnd_woo_cart_but_count( $fragments ) {
    ...
    $fragments['a.alarnd__cart_menu_item'] = ob_get_clean();
...
add_action( 'wp_header', 'ajax_fetch' );
function ajax_fetch() {
?>
...
function data_fetch(){
    ...
    $keyword = isset( $_POST['keyword'] ) && ! empty( $_POST['keyword'] ) ? sanitize_text_field( $_POST['keyword'] ) : "";
    $params = array(
        'posts_per_page' => 15,
        'post_type' => 'product'
    );
    ...
    if( $the_query->have_posts() ) : ?>
        ...
        <!--<h2><?//php esc_html_e( 'Search Results', 'hello-elementor' ); ?></h2>-->
        ...
        <?php endwhile;
        if ( 
            count($product_cats) > 0 && 
            ! empty( $product_cats ) && 
            ! is_wp_error( $product_cats )
        ) {
          foreach ( $product_cats as $term ) {
            $title = $term->name;
            $thumb_id = get_term_meta( $term->term_id, 'thumbnail_id', true );
            $thumb_url = ! empty( $thumb_id ) ? wp_get_attachment_url( $thumb_id, 'thumbnail' ) : get_template_directory_uri() . '/assets/images/icon-placeholder.png';
            ?>
            <a class="dummy-media-object" href="<?php echo esc_url( get_term_link( $term ) ); ?>">
...
?>
    ...
    <div id="morphsearch" class="elem_search_bar morphsearch">
        <div class="morphsearch-form">
            <input type="text" name="s" placeholder="<?php esc_html_e( 'חפשו פה…', 'hello-elementor' ); ?>" id="keyword" class="input_search morphsearch-input" onchange="fetch()" onpaste="fetch()" >
            <!--<button>-->
        ...
        <div class="search_result morphsearch-content" id="datafetch">
			<div class="dummy-column">
				<!--<h2><?//php esc_html_e( 'Products', 'hello-elementor' ); ?></h2>-->
				<div class="searchresult-column-cont">
                    ...
                    <?php else:  ?>
                    <p>
                         <?php esc_html_e( 'No Products', 'hello-elementor' ); ?>
                    </p>
                    <?php endif; ?>
                </div>
			</div>
